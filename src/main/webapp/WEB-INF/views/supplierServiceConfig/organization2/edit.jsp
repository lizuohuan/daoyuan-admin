...
<div style="margin: 15px;">
    ...
    <form class="layui-form" action="" id="formData" ng-controller="controller" ng-cloak>
        ...
        <div class="layui-form-item ">
            ...
            <div class="layui-input-inline">
                <input value="{{organization.inTheEndTime}}" type="number" name="inTheEndTime" lay-verify="required|isValue" placeholder="请输入最晚实做日期" autocomplete="off" class="layui-input" maxlength="2">
            </div>
        ...
        <div class="layui-form-item ">
            ...
            <div class="layui-input-inline">
                <input value="{{organization.remindTime}}" type="number" name="remindTime" lay-verify="required|isValue" onkeyup="onRemindTime(this)" placeholder="请输入提醒日期" autocomplete="off" class="layui-input" maxlength="2">
            </div>
        ...
        <div class="layui-form-item ">
            ...
            <div class="layui-input-inline">
                <div style="width: 400px;cursor: not-allowed;background: #EEEEEE" id="remindContent" class="layui-textarea">
                    <span id="cityName"></span>公积金中心本月变动将于<span id="day">{{organization.remindTime}}</span>号<span id="dot">24</span>点截止，请及时提交变动，避免影响<span id="operationTypeText">XX</span>账单，谢谢。
                </div>
            </div>
        </div>
        ...
        <div class="layui-form-item ">
            <label class="layui-form-label"><span>基数精度填写</span><span class="font-red">*</span></label>
            <div class="layui-input-inline">
        ...
        <div class="layui-form-item ">
            ...
            <div class="layui-input-inline" style="width: 150px;">
                <input value="{{organization.minCardinalNumber}}" type="number" name="minCardinalNumber" placeholder="基数下限" autocomplete="off" class="layui-input" maxlength="10">
            </div>
            <div class="layui-form-mid">-</div>
            <div class="layui-input-inline" style="width: 150px;">
                <input value="{{organization.maxCardinalNumber}}" type="number" name="maxCardinalNumber" placeholder="基数上限" autocomplete="off" class="layui-input" maxlength="10">
            </div>
        </div>
...
<script>
    ...
    webApp.controller("controller", function($scope,$http,$timeout){
        ...
        layui.use(['form', 'layedit'], function() {
            var form = layui.form();
            $("#cityName").html(sessionStorage.getItem("mergerName") + "--" + $scope.organization.organizationName);
            form.verify({
            ...
            $("select[name='operationType'] option").each(function () {
                if ($(this).val() == $scope.organization.operationType) {
                    $(this).attr("selected", true);
                    $("#operationTypeText").html($(this).text());
                }
            });
            $("select[name='computationalAccuracy'] option").each(function () {
                if ($(this).val() == $scope.organization.computationalAccuracy) {
                    $(this).attr("selected", true);
                }
            });
            if ($scope.organization.computationRule == 1) {
                $("select[name=computationalAccuracy]").attr("disabled", true);
            }
            $("select[name='precision'] option").each(function () {
                if ($(this).val() == $scope.organization.precision) {
                    $(this).attr("selected", true);
                }
            });
            $("select[name='computationRule'] option").each(function () {
                if ($(this).val() == $scope.organization.computationRule) {
                    $(this).attr("selected", true);
                }
            });
            form.render();
