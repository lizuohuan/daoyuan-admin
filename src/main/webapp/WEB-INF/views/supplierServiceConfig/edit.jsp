...
<div style="margin: 15px;">
    ...
    <form class="layui-form" action="" id="formData" ng-controller="controller" ng-cloak>
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-inline">
                <input value="{{payPlace.yearAlterMonth}}" type="text" name="yearAlterMonth" lay-verify="isValue2" class="layui-input" placeholder="请输入年度调基月份">
            </div>
        </div>
        ...
        <div class="layui-form-item hide">
            ...
            <div class="layui-input-inline">
                <input value="{{payPlace.inTheEndTime}}" type="number" name="inTheEndTime" placeholder="请输入最晚实做日期" autocomplete="off" class="layui-input" maxlength="2">
            </div>
        ...
        <div class="layui-form-item hide">
            ...
            <div class="layui-input-inline">
                <input value="{{payPlace.remindTime}}" type="number" name="remindTime" onkeyup="onRemindTime(this)" placeholder="请输入提醒日期" autocomplete="off" class="layui-input" maxlength="2">
            </div>
        ...
        <div class="layui-form-item hide">
            ...
            <div class="layui-input-inline">
                <div style="width: 400px;cursor: not-allowed;background: #EEEEEE" id="remindContent" class="layui-textarea">
                    <span id="cityName">{{payPlace.city.mergerName}}</span>本月社保变动将于<span id="day">{{payPlace.remindTime}}</span>号<span id="dot">24</span>点截止，请及时提交变动，避免影响<span id="operationTypeText">XX</span>账单，谢谢。
                </div>
            </div>
        </div>

        <input type="hidden" name="cityId" value="{{payPlace.cityId}}">
...
<script>
    ...
    webApp.controller("controller", function($scope,$http,$timeout){
        ...
        AM.ajaxRequestData("get", false, AM.ip + "/payPlace/getPayPlaceById", {id : $scope.id} , function(result) {
            $scope.payPlace = result.data;
        });
        layui.use(['form', 'layedit'], function() {
            ...
            $("select[name='operationType'] option").each(function () {
                if ($(this).val() == $scope.payPlace.operationType) {
                    $(this).attr("selected", true);
                    $("#operationTypeText").html($(this).text());
                }
            });
            $("select[name='type'] option").each(function () {
                if ($(this).val() == $scope.payPlace.type) {
                    $(this).attr("selected", true);
                }
            });
            selectProvince($scope.payPlace.city.townId);
            selectCity($scope.payPlace.city.townId, $scope.payPlace.cityId);
            if ($scope.payPlace.type == 0) {
            ...
            form.on('submit(demo1)', function(data) {
                ...
                AM.log(data.field);
                AM.ajaxRequestData("post", false, AM.ip + "/payPlace/update", data.field , function(result) {
