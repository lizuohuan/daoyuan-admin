    ...
    <form class="layui-form" action="" id="formData" ng-controller="controller" ng-cloak>
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="text" name="title" value="{{repository.title}}"  lay-verify="required" placeholder="请输入标题" autocomplete="off" class="layui-input" maxlength="100">
            </div>
        </div>
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-inline">
                <input type="text" name="antistop" value="{{repository.antistop}}" lay-verify="required" placeholder="请输入关键词" autocomplete="off" class="layui-input" maxlength="50">
            </div>
        </div>
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-inline">
                <input class="layui-input" value="{{repository.startValidity | date : 'yyyy-MM-dd'}}" name="startValidity" placeholder="开始日" id="start" readonly>
            </div>
            <div class="layui-input-inline">
                <input class="layui-input" value="{{repository.endValidity | date : 'yyyy-MM-dd'}}" name="endValidity" placeholder="截止日" id="end" readonly>
            </div>
        </div>
...
<script>
    ...
    webApp.controller("controller", function($scope,$http,$timeout){
        ...
        layui.use(['form', 'layedit'], function() {
            ...
            //构建一个默认的编辑器
            var index = layedit.build('externalInfo');
            var index2 = layedit.build('interiorInfo');
            //添加富文本编辑器的上传图片接口
            layedit.set({
                uploadImage: {
            ...
            $("input[name=type]").each(function () {
                if ($(this).val() == $scope.repository.type) {
                    $(this).prop("checked", true);
                }
            });

            var roleIds = $scope.repository.roleIds.split(",");
            ...
            form.on('submit(demo1)', function(data) {
                ...
                AM.log(data.field);
                AM.ajaxRequestData("post", false, AM.ip + "/repository/update", data.field , function(result) {
