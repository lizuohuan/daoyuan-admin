...
<div style="margin: 15px;">
    ...
    <form class="layui-form" action="" id="formData" ng-controller="controller" ng-cloak>
        ...
        <div class="layui-form-item ">
            ...
            <div class="layui-input-inline">
                <input type="text" value="{{bankInfo.accountName}}" name="accountName" lay-verify="required" placeholder="请输入银行账户" autocomplete="off" class="layui-input" maxlength="200">
            </div>
        </div>
        ...
        <div class="layui-form-item ">
            ...
            <div class="layui-input-inline">
                <input type="text" value="{{bankInfo.bankAccount}}" name="bankAccount" placeholder="请输入银行账号" autocomplete="off" class="layui-input" maxlength="200">
            </div>
        </div>
        ...
        <div class="layui-form-item ">
            ...
            <div class="layui-input-inline">
                <input type="text" value="{{bankInfo.bankName}}" name="bankName" placeholder="请输入开户行" autocomplete="off" class="layui-input" maxlength="200">
            </div>
        </div>

        <input type="hidden" value="{{bankInfo.companyId}}" name="companyId">
...
<script>
    ...
    webApp.controller("controller", function($scope,$http,$timeout){
        ...
        $scope.bankInfo = JSON.parse(sessionStorage.getItem("bankInfo"));
        AM.log($scope.bankInfo);
        layui.use(['form', 'layedit'], function() {
            var form = layui.form();

            $("input[name=type]").each(function () {
                if ($(this).val() == $scope.bankInfo.type) {
                    $(this).click();
                }
            });

            if ($scope.bankInfo.type == 0) {
            ...
            form.on('submit(demo1)', function(data) {
                data.field.id = $scope.bankInfo.id;
                AM.ajaxRequestData("post", false, AM.ip + "/bankInfo/updateBankInfo", data.field , function(result) {
